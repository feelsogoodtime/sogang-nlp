...
def _get_tagger():
    ...
    if _TAGGER is None:
        try:
            ...
            # Try with explicit dictionary path on Windows
            import os
            import sys
            if sys.platform == "win32":
                # Try common MeCab installation paths
                mecab_paths = [
                ...
                for path in mecab_paths:
                    if os.path.exists(path):
                        try:
                            _TAGGER = MeCab.Tagger(f'-d "{path}"')
                            break
                        except:
                            continue
                if _TAGGER is None:
                    # Use unidic as fallback
                    try:
                        import unidic
                        _TAGGER = MeCab.Tagger(f'-d "{unidic.DICDIR}"')
                    except:
                        _TAGGER = MeCab.Tagger('-r nul -d nul')
            else:
                raise
    return _TAGGER
