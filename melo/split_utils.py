...
def txtsplit(text, desired_length=100, max_length=200):
    ...
    # Preserve currency formatting by temporarily replacing currency patterns
    currency_patterns = []
    currency_re = re.compile(r'(£|\$|¥|€)([0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]{2})?)')
    
    def preserve_currency(match):
        placeholder = f"__CURRENCY_{len(currency_patterns)}__"
        currency_patterns.append(match.group(0))
        return placeholder
    
    text = currency_re.sub(preserve_currency, text)
    
    # Add spaces after punctuation (but not in preserved currency)
